...
fun process_create_after_lowering_steak_req_ok() {
    ...
    state.process_stake(
        id_from_address(POOL_ID),
        id_from_address(ALICE),
        60 * constants::deep_unit(),
        test.ctx(),
    );
    state.process_stake(
        id_from_address(POOL_ID),
        id_from_address(BOB),
        50 * constants::deep_unit(),
        test.ctx(),
    );
...
fun process_cancel_ok() {
    ...
    assert_eq(
        settled,
        balances::new(0, 100 * constants::usdc_unit(), 5_000_000),
    );
    assert_eq(owed, balances::new(0, 0, 0));
...
fun process_cancel_after_modify_epoch_change_ok() {
    ...
    assert_eq(
        settled,
        balances::new(0, 50 * constants::usdc_unit(), 2_500_000),
    );
    assert_eq(owed, balances::new(0, 0, 0));
